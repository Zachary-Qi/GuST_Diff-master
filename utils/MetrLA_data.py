...
DATA_ROOT = Path(os.getenv("IMPUTEST_DATA_ROOT", _repo_root() / "datasets/metr_la"))

def get_metrLa_data(args):

    # 对邻接矩阵进行处理：
    # 读取距离数据
    # distances = pd.read_csv(DATA_ROOT / 'distances_la.csv')

    # # 读取节点ids
    # ids_path = DATA_ROOT / 'sensor_ids_la.txt'
    ...
    # 读取元数据
    df = pd.read_hdf(DATA_ROOT / 'metr_la.h5')
